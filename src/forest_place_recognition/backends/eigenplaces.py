"""EigenPlaces wrapper backend.

Requires the ``eigenplaces`` package (or ``torch.hub`` access) to be
...
def is_available() -> bool:
    """Return True if torch/torchvision are importable (EigenPlaces uses torch.hub)."""
    return _AVAILABLE


class EigenPlaces:
    """Thin wrapper around the EigenPlaces model loaded via ``torch.hub``.
    ...
    def __init__(
        ...
        if not _AVAILABLE:
            raise RuntimeError(
                "EigenPlaces requires torch and torchvision. "
                "Install them first."
            )
